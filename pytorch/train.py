    ...
    for i in range(n_epochs):
        ...
        for j, (data_batch,target_batch) in enumerate(dataloader):
            ...
            nle_value, kl_value, mse_value, bce_dis_original_value, bce_dis_sampled_value,\
                bce_gen_original_value,bce_gen_sampled_value, nllloss = VaeGan.loss(data_target, out,
                                                                                    out_layer_original,
                                                                                    out_layer_predicted,
                                                                                    out_labels_original,
                                                                                    out_labels_sampled,
                                                                                    mus, variances, aux_labels,
                                                                                    aux_labels)
            ...
            loss_discriminator_mean((torch.mean(bce_dis_original_value) + torch.mean(bce_dis_sampled_value)
                                     + torch.mean(nllloss)).data.cpu().numpy()[0])
            ...
            loss_aux_classifier_mean(torch.mean(nllloss).data.cpu().numpy()[0])
